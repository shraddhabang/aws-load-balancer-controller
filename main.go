...
func main() {
	...
	if nlbGatewayEnabled || albGatewayEnabled {
		...
		if controllerCFG.FeatureGates.Enabled(config.NLBGatewayAPI) {
			gwControllerConfig.routeLoader = routeutils.NewLoader(mgr.GetClient(), ctrl.Log.WithName("nlb-route-loader"))
		...
		if controllerCFG.FeatureGates.Enabled(config.ALBGatewayAPI) {
			if gwControllerConfig.routeLoader == nil {
				gwControllerConfig.routeLoader = routeutils.NewLoader(mgr.GetClient(), ctrl.Log.WithName("alb-route-loader"))
			}
