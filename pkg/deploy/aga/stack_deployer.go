...
func NewDefaultStackDeployer(cloud services.Cloud, config config.ControllerConfig, tagPrefix string,
	logger logr.Logger, metricsCollector lbcmetrics.MetricCollector, controllerName string) *defaultStackDeployer {

	trackingProvider := tracking.NewDefaultProvider(tagPrefix, config.ClusterName, tracking.WithRegion(config.AWSConfig.Region))
