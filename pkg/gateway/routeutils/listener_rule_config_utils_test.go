...
func TestFilterListenerRuleConfigBySecret(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Secret reference with explicit namespace",
			secret: &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
			...
			mockSetup: func(ctrl *gomock.Controller) client.Client {
				...
				explicitNamespace := "secret-ns"
				k8sClient.Create(context.Background(), &elbv2gw.ListenerRuleConfiguration{
					ObjectMeta: metav1.ObjectMeta{
						Name:      "config-cross-ns",
						Namespace: "config-ns",
					},
					Spec: elbv2gw.ListenerRuleConfigurationSpec{
						Actions: []elbv2gw.Action{
			...
			expectedCount: 1,
			expectedErr:   nil,
			expectedNames: []string{"config-cross-ns"},
		},
		{
			name: "Secret reference with wrong namespace",
			secret: &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "secret-name",
					Namespace: "wrong-ns",
		...
		},
		{
			...
			mockSetup: func(ctrl *gomock.Controller) client.Client {
				...
				mockClient.EXPECT().List(gomock.Any(), &elbv2gw.ListenerRuleConfigurationList{}).Return(fmt.Errorf("API error"))
				return mockClient
			},
			expectedCount: 0,
...
func TestIsListenerRuleConfigReferencingSecret(t *testing.T) {
	tests := []struct {
		...
		},
		{
					...
					Actions: []elbv2gw.Action{
						{
							...
							AuthenticateOIDCConfig: &elbv2gw.AuthenticateOidcActionConfig{
								...
								Secret: &elbv2gw.Secret{
									Name:      "test-secret",
									Namespace: awssdk.String("secret-ns"),
								},
							},
						},
					},
			...
			secretKey: types.NamespacedName{Name: "test-secret", Namespace: "secret-ns"},
			expected:  true,
		},
		{
		...
		{
			name: "OIDC action with non-matching secret namespace",
			listenerRuleConfig: &elbv2gw.ListenerRuleConfiguration{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "config-1",
					Namespace: "config-ns",
					...
					Actions: []elbv2gw.Action{
						{
							...
							AuthenticateOIDCConfig: &elbv2gw.AuthenticateOidcActionConfig{
								...
								Secret: &elbv2gw.Secret{
									Name:      "test-secret",
									Namespace: awssdk.String("wrong-ns"),
								},
							},
						},
					},
