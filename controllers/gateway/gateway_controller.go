...
func (r *gatewayReconciler) reconcileHelper(ctx context.Context, req reconcile.Request) error {
	...
	for _, ao := range allAddOns {
		if ao.Enabled {
			currentAddOns = append(currentAddOns, ao.Name)
		}
	}
	...
	// To handle addon additions, we need to write the latest add on config back to the Gateway before actually materializing the addon.
	// This is to ensure that we don't orphan an addon if the user removes the addon prior to a successful annotation update.

	if len(addOnAdditions) > 0 {
		err = persistAddOns(ctx, r.k8sClient, gw, addOnAdditions.UnsortedList(), false)
		if err != nil {
			return err
		}
	}
	...
	if lb == nil {
		err = r.reconcileDelete(ctx, gw, stack, allRoutes, secrets)
		if err != nil {
	...
	// To handle addon removals, we have be sure that the addon was completely removed before updating the annotations.
	// By doing so, we have guaranteed that no resources will be orphaned when we update the annotation to remove the annotation,
...
func (r *gatewayReconciler) reconcileDelete(ctx context.Context, gw *gwv1.Gateway, stack core.Stack, routes map[int32][]routeutils.RouteDescriptor, secrets []types.NamespacedName) error {
...
func (r *gatewayReconciler) buildModel(ctx context.Context, gw *gwv1.Gateway, cfg elbv2gw.LoadBalancerConfiguration, listenerToRoute map[int32][]routeutils.RouteDescriptor, currentAddonConfig []addon.Addon) (core.Stack, *elbv2model.LoadBalancer, []addon.AddonMetadata, bool, []types.NamespacedName, error) {
	stack, lb, newAddOnConfig, backendSGRequired, secrets, err := r.modelBuilder.Build(ctx, gw, cfg, listenerToRoute, currentAddonConfig)
	if err != nil {
