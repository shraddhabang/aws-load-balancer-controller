...
func verifyListenerRuleActions(actual, expected []elbv2types.Action) error {
	...
	for i, expectedAction := range expected {
		...
		case elbv2types.ActionTypeEnumAuthenticateOidc:
			...
			// Verify ClientSecret
			if awssdk.ToBool(actualAction.AuthenticateOidcConfig.UseExistingClientSecret) !=
				awssdk.ToBool(expectedAction.AuthenticateOidcConfig.UseExistingClientSecret) {
				return errors.Errorf("ClientSecret mismatch: expected %s, got %s",
					awssdk.ToBool(actualAction.AuthenticateOidcConfig.UseExistingClientSecret),
					awssdk.ToBool(expectedAction.AuthenticateOidcConfig.UseExistingClientSecret))
			}
