...
func newALBResourceStack(dp *appsv1.Deployment, svc *corev1.Service, gwc *gwv1.GatewayClass, gw *gwv1.Gateway, lbc *elbv2gw.LoadBalancerConfiguration, tgc *elbv2gw.TargetGroupConfiguration, lrc *elbv2gw.ListenerRuleConfiguration, httpr []*gwv1.HTTPRoute, secret *testOIDCSecret, baseName string, enablePodReadinessGate bool) *albResourceStack {
...
func (s *albResourceStack) createOIDCSecretWithRBAC(ctx context.Context, f *framework.Framework) error {
	...
	if err := f.K8sClient.Create(ctx, secret); err != nil {
		return err
	}

	// 2. Create Role for secret access
	role := &rbac.Role{
		ObjectMeta: metav1.ObjectMeta{
			Name:      roleName,
			Namespace: s.OIDCSecret.namespace,
	...
	// 3. Create RoleBinding to aws-load-balancer-controller service account
	...
	return f.K8sClient.Create(ctx, roleBinding)
}
