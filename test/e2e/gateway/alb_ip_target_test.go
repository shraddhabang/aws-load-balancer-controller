...
var _ = Describe("test k8s alb gateway using ip targets reconciled by the aws load balancer controller", func() {
	...
	//			err := stack.Deploy(ctx, auxiliaryStack, tf, gwListeners, []*gwv1.HTTPRoute{httpr}, lbcSpec, tgSpec, lrcSpec, nil, true)
	...
	//		By("deploying stack", func() {
	//			err := stack.Deploy(ctx, nil, tf, gwListeners, []*gwv1.HTTPRoute{httpr}, lbcSpec, tgSpec, lrcSpec, nil, true)
	//			Expect(err).NotTo(HaveOccurred())
	//		})
	//
	//		By("checking gateway status for lb dns name", func() {
	...
	Context("with ALB ip target configuration with secure HTTPRoute and and authenticate cognito action", func() {
		...
		It("should provision internet-facing load balancer with authenticate-cognito action", func() {
			...
			By("deploying stack", func() {
				err := stack.Deploy(ctx, nil, tf, gwListeners, []*gwv1.HTTPRoute{httpr}, lbcSpec, tgSpec, lrcSpec, nil, false)
				Expect(err).NotTo(HaveOccurred())
			})
	...
	Context("with ALB ip target configuration with secure HTTPRoute and authenticate oidc action", func() {
		...
		It("should provision internet-facing load balancer with authenticate-oidc action", func() {
			...
			By("deploying stack", func() {
				err := stack.Deploy(ctx, nil, tf, gwListeners, []*gwv1.HTTPRoute{httpr}, lbcSpec, tgSpec, lrcSpec, oidcSecret, false)
				Expect(err).NotTo(HaveOccurred())
			})
